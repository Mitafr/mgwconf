    ...
    async fn handle(client: &Client, app: &Arc<Mutex<A>>, config: &C, e: &IoEvent) -> Result<(), anyhow::Error> {
        ...
        match e {
            IoEvent::GetAllApiClientCredentials => {
                let entities = api::configuration::api_client_credentials_api::api_credentials_info_get(
                    &Configuration {
            ...
            IoEvent::PostApiClientCredential(entity) => {
                ...
                api::configuration::api_client_credentials_api::api_credentials_info_create(
                    &Configuration {
                        base_path: format!("{}/swift/mgw/mgw-configuration-api/2.0.0", base_url(config)),
                        client: client.clone(),
                        api_key: Some(ApiKey {
